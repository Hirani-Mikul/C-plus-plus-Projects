							...
							const char	*helper_executable/*=NULL*/)
{
	...
	BOOL bRet = CreateProcess(NULL, (LPWSTR)cmdline,NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
	if (!bRet)
	{	// on failure - try to get the path from the environment
		...
		if (path)
		{
			...
			bRet = CreateProcess(NULL, (LPWSTR)cmdline,NULL,NULL,FALSE,CREATE_NEW_CONSOLE,NULL,NULL,&si,&pi);
		}
		if (!bRet)
		{
			MessageBox(NULL, (LPCWSTR)"Helper executable not found", (LPCWSTR)"ConsoleLogger failed",MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	if (!bConnected)
	{
		MessageBox(NULL, (LPCWSTR)"ConnectNamedPipe failed", (LPCWSTR)"ConsoleLogger failed",MB_OK);
	...
	{
		MessageBox(NULL, (LPCWSTR)"WriteFile failed(1)", (LPCWSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		...
		if (cbWritten!=strlen(buffer))
		{
			MessageBox(NULL, (LPCWSTR)"WriteFile failed(2)", (LPCWSTR)"ConsoleLogger failed",MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe=INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten!=1)
	{
		MessageBox(NULL, (LPCWSTR)"WriteFile failed(3)", (LPCWSTR)"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
